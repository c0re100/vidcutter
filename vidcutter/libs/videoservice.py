...
class VideoService(QObject):
    ...
    def cut(self, source: str, output: str, frametime: str, duration: str, allstreams: bool = True, gifOutput: bool = False, mp4Output: bool = False, webmOutput: bool = False) -> bool:
        ...
        webpArgs = '-y -ss {0} -i "{1}" -t {2} -c:v libwebp_anim -compression_level 5 -q:v 80 -loop 0 -pix_fmt bgra ' + \
            '-vf "scale=iw*min(1\,min(640/iw\,360/ih)):-2:flags=lanczos,fps=fps=23" "{3}"'
        ...
        # Output .webp format
        if gifOutput and not os.path.exists(webp_dir):
            self.cmdExec(self.backend, webpArgs.format(frametime, source, duration,
                                                       QDir.fromNativeSeparators(output.replace(".mp4", ".webp"))))
    ...
    def cmdExec(self, cmd: str, args: str = None, output: bool = False):
        ...
        if self.proc.state() == QProcess.NotRunning:
            ...
            if cmd == self.gifski and os.name == 'posix':
                os.system(cmd+' '+args)
            else:
